...
public class FileStreamProvider : FileSystemProviderBase {
    ...
    List<FileSystemData> DataSource {
        get
        {
            var obj = HttpContext.Current.Session["DataSource"];
            if (obj == null) { obj = HttpContext.Current.Session["DataSource"] = CreateDataSource(); }
            return (List<FileSystemData>)obj;
        }
        set
        {
            HttpContext.Current.Session["DataSource"] = value;
        }
    }
    ...
    public override void UploadFile(FileManagerFolder folder, string fileName, Stream content) {
        ...
        DataSource.Add(new FileSystemData() {
            ...
            LastWriteTime = DateTime.Now,
            Name = fileName,
            LastUpdateBy = cs.CurUserName,
            Data = cs.ReadToEnd(content),
    ...
    private FileSystemData GetRootFolder() {
        return DataSource
            .Where(x => x.IsFolder && x.ParentId == -1)
            .FirstOrDefault();
    }
