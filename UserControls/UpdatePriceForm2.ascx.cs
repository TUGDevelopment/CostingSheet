...
public partial class UserControls_UploadcostForm : MasterUserControl
{
    ...
    void _alertmail(DataRow dr, string status)
    {
        ...
        foreach (DataRow row in dt.Rows)
            if (string.Format("{0}", dr["RequestNo"].ToString().Substring(3, 1)) == "1")
            {
				...
				sender = sender.ToLower().Replace("wipada.termlertmanuswong@thaiunion.com", "");
				MailCc = MailCc.ToLower().Replace("wipada.termlertmanuswong@thaiunion.com", "");
				sender = sender.Replace("GPCUser04", "");
				MailCc = MailCc.Replace("GPCUser04", "");
                        ...
                        if (!string.IsNullOrEmpty(value))
                        {
                            ...
                            if (value.Length == 18 && value.Substring(0, 1) == "3")
                            {
                                ...
                                foreach (DataRow row in myresult.Rows)
                                    array[3] = string.Format("{0}", row["MarketingNumber"]);
                            }
                            else
                                if (GetExcelUserType(value.Substring(0, 2)) != "0")
                                array[3] = string.Format("{0}", value);
                            else if (getuserType() != "0")
                                array[3] = string.Format("{0}", value);
                            array[4] = string.Format("{0}", 0);
                            array[5] = i;
    ...
    void updateEditCosting(DataRow dr, List<string> myresult)
    {
        using (SqlConnection con = new SqlConnection(strConn))
        {
            ...
            cmd.Parameters.AddWithValue("@CostingSheet", dr["CostingSheet"].ToString());
            cmd.Parameters.AddWithValue("@Material", dr["Code"].ToString().Trim());
            cmd.Parameters.AddWithValue("@Series", dr["Series"].ToString());
            cmd.Parameters.AddWithValue("@user", hfuser["user_name"]);
            cmd.Connection = con;
            con.Open();
            cmd.ExecuteNonQuery();
    ...
    void getselectmapCost(object keyValue, ASPxGridView g)
    {
        ...
        switch (args[1])
        {
            case "add":
                SqlParameter[] param = {new SqlParameter("@GeID",string.Format("{0}", hfGeID["GeID"])),
                        new SqlParameter("@user", hfuser["user_name"]),
                        new SqlParameter("@param", args[2])};
                table = GetRelatedResources("spGetUpdateCost", param);
                foreach (DataRow dr in table.Rows)
                    using (SqlConnection con = new SqlConnection(strConn))
                    {
                        SqlCommand cmd = new SqlCommand();
        ...
        DataView view = new DataView(table); var list = new List<string>();
