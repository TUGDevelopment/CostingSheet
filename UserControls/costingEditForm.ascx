<%@ Control Language="C#" AutoEventWireup="true" CodeFile="CostingEditForm.ascx.cs" Inherits="UserControls_CostingEditForm" %>
   <script type="text/javascript">
       ...
       function OnEndCallback(s, e) {
           ...
           if (!grid.cpKeyValue)
               return;
           grid.PerformCallback(grid.cpKeyValue);
       ...
       function OnFileUploadComplete(s, e) {
           ...
           if (e.callbackData) {
               ...
               ClientReference.SetValue(fileData[1]);
       ...
       function OnBatchEditEndEditing(s, e) {
           ...
           var pk = [1,2,3,4,5,6,7,8,9];
           if (pk.some(x => tcDemos.GetActiveTabIndex())) {
               testgrid_OnBatchEditEndEditing(s, e);
           }else if (focusedfieldName)
               calculator(s);
       ...
       function insertSelection(command) {
           ...
           grid.PerformCallback("Insert|" + tcDemos.GetActiveTabIndex());
       }
       ...
       function OnConfirm() {
           // JS Code to handle post confirm operation
           grid.PerformCallback('copy|' + tcDemos.GetActiveTabIndex());
       }
       ...
       function LoadName(symbol) {
           ...
           //grid.AutoFilterByColumn('Formula', symbol);
           //testgrid.AutoFilterByColumn('Formula', symbol);
           grid.PerformCallback(value);
           //testgrid.PerformCallback(value);
       }
       ...
       function OnActiveTabChanged(s, e) {
           ...
           grid.SetVisible(selectedIndex != 11);
           //grid.PerformCallback('symbol|' + selectedIndex);
           //testgrid.SetVisible(b);
           //if (selectedIndex == 7)
           //    fileManager.PerformCallback(['load', 0].join("|"));
           //else {
           //    grid.PerformCallback('TabChanged|' + selectedIndex);
           //}
           grid.PerformCallback('TabChanged|' + selectedIndex);
          
           formLayout.GetItemByName("Attached").SetVisible(selectedIndex == 12);
       ...
       function calculator(s) {
               ...
               if (ClientCompany.GetText() == "101") {
                   s.GetValuesOnCustomCallback("PackSize|" + s.batchEditApi.GetCellValue(curentEditingIndex, "Formula"), function (r) {
                       if (!r)
                           return;
                       PackSize = r["PackSize"];
                   });
               } else
                   PackSize = ClientPackSize.GetText()
               var total = parseFloat(GUnit) / 1000;
               if (total != null || Yield != null || BaseUnit != null) {
                   PriceOfCarton = (parseFloat(total) * parseFloat(BaseUnit) * parseFloat(PackSize) / (parseFloat(Yield) / 100));
               }
       ...
       function OnButtonClick2(evt) {
           debugger;
           var Component = grid.batchEditApi.GetCellValue(curentEditingIndex, "SubType");
           if (Component == "Secondary Packaging")
               evt = "SecPackageCode";
           OnButtonClick(evt);
       }
       function OnButtonClick(evt) {
           ...
           else if (evt == "PackageCode" || evt == "SecPackageCode" || evt == "Labor" || evt == "OHRate" || evt == "SGA" || evt == "DL") {
       ...
       function OnGetValuesOnCustomCallbackComplete(values) {
           if (!values)
               return;
           debugger;
           if (values["view"] == "SecPackageCode" && tcDemos.GetActiveTabIndex()==10)
           {
               var g = grid.batchEditApi;
               g.SetCellValue(curentEditingIndex, "Code", values["ID"]);
               g.SetCellValue(curentEditingIndex, "Name", values["Name"]);
               g.SetCellValue(curentEditingIndex, "Quantity", 1);
               g.SetCellValue(curentEditingIndex, "Amount", values["Rate"]);
               g.SetCellValue(curentEditingIndex, "PriceUnit", values["Rate"]);
               g.SetCellValue(curentEditingIndex, "SellingUnit", values["Unit"]);
               window.setTimeout(function () {
                   var count_value = GetChangesCount(grid.batchEditApi);
                   if (count_value > 0)
                       grid.UpdateEdit();
           ...
           else if (values["view"] == "SecPackageCode") {
           ...
           else if (values["view"] == "SGA") {
               tbSGAQuantity.SetText(values["Q"]);
               cmbSGA.SetValue(values["Name"]);
               tbSGARate.SetText(values["Rate"]);
               cmbSGAType.SetValue(values["Unit"]);
           }
           else if (values["view"] == "OHRate" || values["view"] == "OH") {
               tbOHQuantity.SetText(values["Q"]);
               cmbOHRate.SetValue(values["Name"]);
               tbOHRate.SetText(values["Rate"]);
               cmbOHType.SetValue(values["Unit"]);
           }
           else if (values["view"] == "DL") {
               cmbLabor.SetValue(values["Name"]);
               tbLaborRate.SetText(values["Rate"]);
               cmbLaborType.SetValue(values["Unit"]);
       ...
       function RequestOnTextChanged(r) {
           ...
           tbCanSize.SetText(r["CanSize"]);
           ClientNetweight.SetText(r["NetWeight"]);
           ClientNetUnit.SetText(r["WeightUnit"]);
           ...
           //alert(value);
           //if (compa == '101') {
           //    ClientNetweight.SetEnabled(false);
           //    ClientNetUnit.SetEnabled(false);
           //    CmbPackaging.SetEnabled(false);
           //    ClientPackSize.SetEnabled(false);
           //} else {
               if (r["RequestType"] == '2')
                   confirm_value(r["ID"]);
               //if (confirm("Are you sure?")) {
       ...
       function BuildEditPopup(r) {
           ...
           if (r == "Request")
               if (hfRequestNo.Get("ID") != "")
                   ClientReceiver.PerformCallback('load|' + hfRequestNo.Get("ID"));


       }
       function OnToolbarItemClick(s, e) {
           //debugger;
           if (e.item.name == "Request" && tcDemos.GetActiveTabIndex() == 9)
               BuildEditPopup("Request");
           if (e.item.name == "New")
               if (tcDemos.GetActiveTabIndex() == 10)
                   grid.PerformCallback("Insert|" + tcDemos.GetActiveTabIndex());
               else
                   insertSelection('new');
      ...
      function combo_SelectedIndexChanged(s, e){
       ...
       function onStartEdit(s, e) {
           ...
           if (tcDemos.GetActiveTabIndex() == 0)
               editpopup.ShowAtElement(grid.GetMainElement());
       }
       function onShown(s, e) {
           dtSAPMaterial.SetText(grid.batchEditApi.GetCellValue(visibleIndex, "SAPMaterial"));
           tbGUnit.SetText(grid.batchEditApi.GetCellValue(visibleIndex, "GUnit"));
       }
       function onAcceptClick(s, e) {
           //grid.batchEditApi.SetCellValue(visibleIndex, "SAPMaterial", dtSAPMaterial.GetText());
           var view = Demo.DemoState.View;
           EditPopup.Hide();
           //if (view == "MailList") {
           //    //ClientCostGrid.PerformCallback("Request|0");
           //    Demo.DoCallback(ClientCostGrid, function () {
           //        ClientCostGrid.PerformCallback(["Request", "0"].join("|"));
           //    });
       ...
       function ApplyFilter() {
           ...
           for (var i = 0; i < grid.GetColumnsCount(); i++)
               if (grid.GetAutoFilterEditor(i) != null) {
                   if (filterCondition != "")
                       andOperator = "And "
                   var editor = eval("filterRow_" + grid.GetColumn(i).fieldName);
                   if (editor)
                       if (editor.GetText() != "")
       ...
       function setGridHeaderCheckboxes(s, e) {
           //cbAll
           debugger;
           var indexes = ClientCostGrid.cpIndexesSelected;
           //cbAll.SetChecked(s.GetSelectedRowCount() == Object.size(indexes));
           var view = Demo.DemoState.View;
           var RowCount = ClientCostGrid.GetSelectedRowCount();
           //cbPage
           var allEnabledRowsOnPageSelected = true;
           var indexes = ClientCostGrid.cpIndexesUnselected;
           var topVisibleIndex = ClientCostGrid.GetTopVisibleIndex();
           for (var i = topVisibleIndex; i < topVisibleIndex + ClientCostGrid.cpPageSize; i++) {
               if (indexes.indexOf(i) == -1)
                   if (!ClientCostGrid.IsRowSelectedOnPage(i)) allEnabledRowsOnPageSelected = false;
           }
           if (hfsublevel.Get("sublevel") == "1" || hfsublevel.Get("sublevel") == "2" || hfsublevel.Get("sublevel") == "17") {
               ClientActionMenu.GetItemByName("Request").SetVisible(view == "MailList" && RowCount > 0);
           } else {
...
<dx:ASPxHiddenField ID="hfsublevel" runat="server" ClientInstanceName="hfsublevel"/>
...
<dx:ASPxGridView runat="server" ID="gridData" KeyFieldName="ID;UniqueColumn" DataSourceID="dsgv" ClientInstanceName="ClientCostGrid" Width="100%" 
        AutoGenerateColumns="true" OnCustomCallback="gridData_CustomCallback" OnCustomButtonCallback="gridData_CustomButtonCallback"
        OnCustomDataCallback="gridData_CustomDataCallback" OnDataBound="gridData_DataBound" 
    ...
    <Columns>
        ...
        <dx:GridViewDataTextColumn FieldName="RequestNo" Caption="TRF No" GroupIndex="0"/>
	...
	<SettingsDataSecurity AllowReadUnlistedFieldsFromClientApi="True" />
        ...
        <ClientSideEvents RowClick="Demo.ClientCostGrid_RowClick"
            ContextMenuItemClick="function(s,e) { OnContextMenuItemClick(s, e); }" SelectionChanged="OnGridSelectionChanged"
            ...
            <dx:ASPxFormLayout ID="EmployeeEditFormLayout" runat="server" AlignItemCaptionsInAllGroups="True">
                ...
                <Items>
                     <dx:LayoutGroup Caption="Calculate" ColCount="4" GroupBoxDecoration="None" UseDefaultPaddings="false">
                        ...
                        <Items>
                            <dx:LayoutItem Caption="Zone/Country" CaptionSettings-Location="Left">
                                <LayoutItemNestedControlCollection>
                                    <dx:LayoutItemNestedControlContainer>
                                        <dx:ASPxComboBox ID="CmbCountry" runat="server" DataSourceID="dsCountry" ValueField="Code" TextField="Name"
                            ...
                            <dx:LayoutItem Caption="R&D PIC">
                                <LayoutItemNestedControlCollection>
                                    <dx:LayoutItemNestedControlContainer>
                                        <dx:ASPxComboBox ID="CmbReceiver" ClientInstanceName="ClientReceiver" OnCallback="CmbReceiver_Callback"
                                        ValueField="ID" TextField="Name" Width="100%" runat="server">
                                    </dx:ASPxComboBox>
                                    </dx:LayoutItemNestedControlContainer>
                                </LayoutItemNestedControlCollection>
                            </dx:LayoutItem>
    ...
    <dx:ASPxCallbackPanel ID="FormPanel" runat="server" RenderMode="Div" ClientVisible="false" ScrollBars="Vertical" ClientInstanceName="ClientCostFormPanel">
        ...
        <PanelCollection>
            ...
            <Items>
                 <dx:LayoutGroup Caption="Costing Sheet" ColCount="4" GroupBoxDecoration="Box" UseDefaultPaddings="false" Paddings-PaddingTop="10">
                    ...
                    <Items>
                        ...
                        <dx:LayoutItem Caption="Net weight" VerticalAlign="Middle">
                            <SpanRules>
                               ...
                               <LayoutItemNestedControlCollection>
                                   <dx:LayoutItemNestedControlContainer runat="server" SupportsDisabledAttribute="True">
                                       <table>
                                           <tr>
                                               <td><dx:ASPxTextBox ID="tbNetweight" runat="server" ClientInstanceName="ClientNetweight" Width="80px">
                                                   <%--<ValidationSettings Display="Dynamic" RequiredField-IsRequired="true" ValidationGroup="group1"/>--%>
                                               ...
                                               <td>&nbsp;</td>
                                               <td><dx:ASPxComboBox ID="CmbNetUnit" runat="server" ClientInstanceName="ClientNetUnit" NullText="Select Unit..." Width="80px">
                           ...
                           <dx:LayoutItem Caption="VP Code" VerticalAlign="Middle">
                            <SpanRules>
                                <dx:SpanRule ColumnSpan="1" RowSpan="1" BreakpointName="S"></dx:SpanRule>
                                <dx:SpanRule ColumnSpan="2" RowSpan="1" BreakpointName="M"></dx:SpanRule>
                            </SpanRules>
                            <LayoutItemNestedControlCollection>
                                <dx:LayoutItemNestedControlContainer>
                                    <dx:ASPxTextBox runat="server" ID="tbVarietyPack" ClientInstanceName="tbVarietyPack" ToolTip="VarietyPack">
                                    </dx:ASPxTextBox>
                                </dx:LayoutItemNestedControlContainer>
                            </LayoutItemNestedControlCollection>
                        </dx:LayoutItem> 
                       ...
                       <dx:LayoutGroup Caption="Upcharge" ColCount="3">
	                    ...
	                    <dx:LayoutGroup Caption="Labor&Overhead" ColCount="1" Width="80%">
                            <Items>
                                ...
                                <dx:LayoutGroup Caption="labor" ColCount="2" Width="80%">
                                    <Items>
                                    <dx:LayoutItem Caption="Labor">
                                            <LayoutItemNestedControlCollection>
                                                <dx:LayoutItemNestedControlContainer>
                                                <dx:ASPxButtonEdit ID="cmbLabor" ClientInstanceName="cmbLabor"
                                                Width="100%" runat="server">
                                                    <Buttons>
                                                        <dx:EditButton>
                                                        </dx:EditButton>
                                                    </Buttons>
                                                        <ClientSideEvents ButtonClick="function(s, e) { OnButtonClick('DL'); }" />
                                                </dx:ASPxButtonEdit>
                                                </dx:LayoutItemNestedControlContainer>
                                            </LayoutItemNestedControlCollection>
                                      </dx:LayoutItem>
                                    ...
                                    <LayoutItemNestedControlCollection>
                                        <dx:LayoutItemNestedControlContainer>
                                            <dx:ASPxTextBox runat="server" ID="tbLaborQuantity" ClientInstanceName="tbLaborQuantity">
                                            </dx:ASPxTextBox>
                                    </dx:LayoutItemNestedControlContainer>
                                </LayoutItemNestedControlCollection>
                                </dx:LayoutItem>
                                        ...
                                        <LayoutItemNestedControlCollection>
                                            <dx:LayoutItemNestedControlContainer>
                                                <table>
                                                    <tr>
                                                        <td>
                                                            <dx:ASPxTextBox runat="server" ID="tbLaborRate" ClientInstanceName="tbLaborRate"/>
                                                        </td>
                                                        <td>&nbsp;</td>
                                                        <td>
                                                            <dx:ASPxComboBox ID="cmbLaborType" runat="server" Width="35px" ClientInstanceName="cmbLaborType"
                                                                DataSourceID="dsLOHBase" ValueField="value" TextField="value"/>
                                                        </td>
                                                    </tr>
                                                </table>
                                ...
                                <dx:LayoutGroup Caption="OH" ColCount="2" Width="80%">
                                    <Items>
                                    <dx:LayoutItem Caption="OH">
                                            <LayoutItemNestedControlCollection>
                                                <dx:LayoutItemNestedControlContainer>
                                                <dx:ASPxButtonEdit ID="cmbOHRate" ClientInstanceName="cmbOHRate"
                                                Width="100%" runat="server">
                                                    <Buttons>
                                                        <dx:EditButton>
                                                        </dx:EditButton>
                                                    </Buttons>
                                                        <ClientSideEvents ButtonClick="function(s, e) { OnButtonClick('OHRate'); }" />
                                                </dx:ASPxButtonEdit>
                                                </dx:LayoutItemNestedControlContainer>
                                            </LayoutItemNestedControlCollection>
                                      </dx:LayoutItem>
                                        ...
                                        <LayoutItemNestedControlCollection>
                                            <dx:LayoutItemNestedControlContainer>
                                                <dx:ASPxTextBox runat="server" ID="tbOHQuantity" ClientInstanceName="tbOHQuantity">
                                                </dx:ASPxTextBox>
                                        </dx:LayoutItemNestedControlContainer>
                                    </LayoutItemNestedControlCollection>
                                    </dx:LayoutItem>
                                        ...
                                        <LayoutItemNestedControlCollection>
                                            <dx:LayoutItemNestedControlContainer>
                                                <table>
                                                    <tr>
                                                        <td>
                                                            <dx:ASPxTextBox runat="server" ID="tbOHRate" ClientInstanceName="tbOHRate"/>
                                                        </td>
                                                        <td>&nbsp;</td>
                                                        <td>
                                                            <dx:ASPxComboBox ID="cmbOHType" runat="server" Width="35px" ClientInstanceName="cmbOHType"
                                                                DataSourceID="dsLOHBase" ValueField="value" TextField="value"/>
                                                        </td>
                                                    </tr>
                                                </table>
                                ...
                                <dx:LayoutGroup Caption="SG &amp; A" ColCount="2" Width="80%">
                                    <Items>
                                    <dx:LayoutItem Caption="SG &amp; A">
                                            <LayoutItemNestedControlCollection>
                                                <dx:LayoutItemNestedControlContainer>
                                                <dx:ASPxButtonEdit ID="cmbSGA" ClientInstanceName="cmbSGA"
                                                Width="100%" runat="server">
                                                    <Buttons>
                                                        <dx:EditButton>
                                                        </dx:EditButton>
                                                    </Buttons>
                                                        <ClientSideEvents ButtonClick="function(s, e) { OnButtonClick('SGA'); }" />
                                                </dx:ASPxButtonEdit>
                                                </dx:LayoutItemNestedControlContainer>
                                            </LayoutItemNestedControlCollection>
                                      </dx:LayoutItem>
                                     <dx:LayoutItem Caption="Quantity">
                                        <LayoutItemNestedControlCollection>
                                            <dx:LayoutItemNestedControlContainer>
                                                <dx:ASPxTextBox runat="server" ID="tbSGAQuantity" ClientInstanceName="tbSGAQuantity">
                                                </dx:ASPxTextBox>
                                        </dx:LayoutItemNestedControlContainer>
                                    </LayoutItemNestedControlCollection>
                                    </dx:LayoutItem>
                                        ...
                                        <LayoutItemNestedControlCollection>
                                            <dx:LayoutItemNestedControlContainer>
                                                <table>
                                                    <tr>
                                                        <td>
                                                            <dx:ASPxTextBox runat="server" ID="tbSGARate" ClientInstanceName="tbSGARate"/>
                                                        </td>
                                                        <td>&nbsp;</td>
                                                        <td>
                                                            <dx:ASPxComboBox ID="cmbSGAType" runat="server" Width="35px" ClientInstanceName="cmbSGAType"
                                                                DataSourceID="dsLOHBase" ValueField="value" TextField="value"/>
                                                        </td>
                                                    </tr>
                                                </table>
                            ...
                            <dx:LayoutGroup Caption="Header" ColCount="1" Width="80%"/>
                            ...
                            <dx:LayoutGroup Caption="Attached file">
                                ...
                                <dx:LayoutItem Caption="" Name="Attached" ClientVisible="false" CaptionSettings-Location="Top">
                                <LayoutItemNestedControlCollection>
                                    <dx:LayoutItemNestedControlContainer>
                                    ...
                                    <PanelCollection>
                                    <dx:PanelContent runat="server">
                                    <dx:ASPxFileManager ID="fileManager" runat="server" CustomFileSystemProviderTypeName="FileStreamProvider" Height="240px" 
                                            OnFileDownloading="OnFileDownloading" OnCustomCallback="OnCustomCallback"
                          ...
                          <SpanRules>
                            ...
                            <LayoutItemNestedControlCollection>
                                ...
                                <dx:ASPxGridView ID="grid" ClientInstanceName="grid" runat="server" Width="100%" OnCustomCallback="grid_CustomCallback"
                                    SettingsPager-PageSize="60" OnDataBinding="grid_DataBinding" EnableViewState="false" KeyFieldName="ID" 
                                    OnCustomDataCallback="grid_CustomDataCallback" OnBatchUpdate="grid_BatchUpdate" OnCommandButtonInitialize="grid_CommandButtonInitialize"
                                    ...
                                    <dx:GridViewToolbar Name="toolbar">
                                        ...
                                        <Items>
                                            ...
                                            <dx:GridViewToolbarItem Text="Undo" Name="Undo" Image-IconID="actions_resetchanges_16x16devav" />
					                ...
					                <dx:GridViewCommandColumn  ShowClearFilterButton="true" Width="42px" ButtonRenderMode="Image"
                                        ...
                                        <%--<dx:GridViewDataComboBoxColumn FieldName="RawMaterial">
                                            <PropertiesComboBox EnableCallbackMode="true" CallbackPageSize="20" DataSourceID="dsMaterial" 
                                            TextFormatString="{0}"  ValueField="Material" DropDownStyle="DropDown">
                                                <Columns>                                            
                                                    <dx:ListBoxColumn FieldName="Material" />
                                                    <dx:ListBoxColumn FieldName="Name" />
                                    ...
                                    <ClientSideEvents BatchEditEndEditing="OnBatchEditEndEditing" BatchEditStartEditing="OnBatchEditStartEditing" 
					                    Init="OnInitGridBatch" FocusedCellChanging="onFocusedCellChanging" ToolbarItemClick="OnToolbarItemClick" EndCallback="OnEndCallback" />      
                                    ...
                                    <SettingsSearchPanel ColumnNames="" Visible="false" />
		                            <Settings ShowGroupPanel="false" VerticalScrollBarMode="Auto" ShowVerticalScrollBar="true" 
                                    ...
                                    <SettingsBehavior AutoExpandAllGroups="true" EnableRowHotTrack="True" ColumnResizeMode="Control" 
                                        EnableCustomizationWindow="true" AllowFocusedRow="true" />
					                <SettingsPager PageSize="50">
						            <PageSizeItemSettings Visible="true" ShowAllItem="true"  AllItemText="All Records" />
					                </SettingsPager>
		                            <Styles>
                       ...
                       <dx:LayoutItem Caption="EventLog" Width="100%" Name="EventLog" CaptionStyle-Font-Bold="true">
                                <LayoutItemNestedControlCollection>
                                    <dx:LayoutItemNestedControlContainer>
                                        ...
                                        <div id="divScroll" style="overflow: scroll; height: auto; width: auto;">
    ...
    <%--<dx:ASPxPopupControl ID="editpopup" runat="server" ClientInstanceName="editpopup" CloseAction="OuterMouseClick"
            Width="400px" Height="600px" AllowResize="true">
            ...
            <dx:PopupControlContentControl>
                ...
                <Items>
                     <dx:LayoutGroup Caption="Calculate" ColCount="4" GroupBoxDecoration="None" UseDefaultPaddings="false">
                        ...
                        <Items>
                            <dx:LayoutItem Caption="Component">
                                <LayoutItemNestedControlCollection>
                                    <dx:LayoutItemNestedControlContainer>
                                        <dx:ASPxComboBox ID="CmdComponent" runat="server" />
                                    </dx:LayoutItemNestedControlContainer>
                                </LayoutItemNestedControlCollection>
                            </dx:LayoutItem>
                            <dx:LayoutItem Caption="SubType">
                                <LayoutItemNestedControlCollection>
                                    <dx:LayoutItemNestedControlContainer>
                                        <dx:ASPxComboBox ID="CmbSubType" runat="server" />
                                    </dx:LayoutItemNestedControlContainer>
                                </LayoutItemNestedControlCollection>
                            </dx:LayoutItem>
                            <dx:LayoutItem Caption="SAPMaterial">
                                <LayoutItemNestedControlCollection>
                                    <dx:LayoutItemNestedControlContainer>
                                        <dx:ASPxButtonEdit ID="dtSAPMaterial" runat="server">
                                            <Buttons>
                                                    <dx:EditButton />
                                                </Buttons>
                                                <ClientSideEvents ButtonClick="function(s, e) { OnButtonClick('SAPMaterial'); }" />
                            ...
                            <dx:LayoutItem Caption="GUnit">
                                <LayoutItemNestedControlCollection>
                                    <dx:LayoutItemNestedControlContainer>
                                        <dx:ASPxTextBox ID="tbGUnit" ClientInstanceName="tbGUnit" runat="server" />
                                    </dx:LayoutItemNestedControlContainer>
                                </LayoutItemNestedControlCollection>
                            </dx:LayoutItem>
                            <dx:LayoutItem Caption="Yield">
                                <LayoutItemNestedControlCollection>
                                    <dx:LayoutItemNestedControlContainer>
                                        <dx:ASPxTextBox ID="tbYield" ClientInstanceName="tbYield" runat="server" />
                                    </dx:LayoutItemNestedControlContainer>
                                </LayoutItemNestedControlCollection>
                            </dx:LayoutItem>
                            <dx:LayoutItem Caption="RawMaterial">
                                <LayoutItemNestedControlCollection>
                                    <dx:LayoutItemNestedControlContainer>
                                        <dx:ASPxButtonEdit ID="btRawMaterial" runat="server">
                                            <Buttons>
                                                    <dx:EditButton />
                                                </Buttons>
                                                <ClientSideEvents ButtonClick="function(s, e) { OnButtonClick('RawMaterial'); }" />
                            ...
                            <dx:LayoutItem Caption="PriceOfUnit">
                                <LayoutItemNestedControlCollection>
                                    <dx:LayoutItemNestedControlContainer>
                                        <dx:ASPxTextBox ID="tbPriceOfUnit" ClientInstanceName="tbPriceOfUnit" runat="server" />
                                    </dx:LayoutItemNestedControlContainer>
                                </LayoutItemNestedControlCollection>
                            </dx:LayoutItem>
                            <dx:LayoutItem Caption="Currency">
                                <LayoutItemNestedControlCollection>
                                    <dx:LayoutItemNestedControlContainer>
                                        <dx:ASPxComboBox ID="CmbCurrency" ClientInstanceName="CmbCurrency" runat="server">
                                            <Columns>
                                                <dx:ListBoxColumn FieldName="Value" />
                                            </Columns>
                                            <ClientSideEvents SelectedIndexChanged="rate_SelectedIndexChaged" />
                            ...
                            <dx:LayoutItem Caption="Unit">
                                <LayoutItemNestedControlCollection>
                                    <dx:LayoutItemNestedControlContainer>
                                        <dx:ASPxComboBox ID="CmbUnit" ClientInstanceName="CmbUnit" 
                                            TextFormatString="{0}" DataSourceID="dsUnit" runat="server" ValueField="Value">
                                            <Columns>
                                                <dx:ListBoxColumn FieldName="Value" />
                                            </Columns>
                                            <ClientSideEvents SelectedIndexChanged="rate_SelectedIndexChaged" />
                            ...
                            <dx:LayoutItem Caption="ExchangeRate">
                                <LayoutItemNestedControlCollection>
                                    <dx:LayoutItemNestedControlContainer>
                                        <dx:ASPxTextBox ID="tbExchangeRate" runat="server" />
                                    </dx:LayoutItemNestedControlContainer>
                                </LayoutItemNestedControlCollection>
                            </dx:LayoutItem>
                            <dx:LayoutItem Caption="BaseUnit">
                                <LayoutItemNestedControlCollection>
                                    <dx:LayoutItemNestedControlContainer>
                                        <dx:ASPxTextBox ID="tbBaseUnit" runat="server" />
                                    </dx:LayoutItemNestedControlContainer>
                                </LayoutItemNestedControlCollection>
                            </dx:LayoutItem>
                            <dx:LayoutItem Caption="PriceOfCarton">
                                <LayoutItemNestedControlCollection>
                                    <dx:LayoutItemNestedControlContainer>
                                        <dx:ASPxTextBox ID="tbPriceOfCarton" runat="server" />
                                    </dx:LayoutItemNestedControlContainer>
                                </LayoutItemNestedControlCollection>
                            </dx:LayoutItem>
                            <dx:LayoutItem Caption="Remark">
                                <LayoutItemNestedControlCollection>
                                    <dx:LayoutItemNestedControlContainer>
                                        <dx:ASPxMemo  ID="mRemark" runat="server" />
                                    </dx:LayoutItemNestedControlContainer>
                                </LayoutItemNestedControlCollection>
                            </dx:LayoutItem>
                            <dx:LayoutItem Caption="LBOh">
                                <LayoutItemNestedControlCollection>
                                    <dx:LayoutItemNestedControlContainer>
                                        <dx:ASPxButtonEdit ID="btLBOh" runat="server">
                                            <Buttons>
                                                    <dx:EditButton />
                                                </Buttons>
                                                <ClientSideEvents ButtonClick="function(s, e) { OnButtonClick('LBOh'); }" />
                            ...
                            <dx:LayoutItem Caption="LBRate">
                                <LayoutItemNestedControlCollection>
                                    <dx:LayoutItemNestedControlContainer>
                                        <dx:ASPxTextBox ID="tbLBRate" runat="server" />
                                    </dx:LayoutItemNestedControlContainer>
                                </LayoutItemNestedControlCollection>
                            </dx:LayoutItem>
                        </Items>
...
<%--    <asp:SqlDataSource ID="dsLaborOverhead" runat="server" ConnectionString="<%$ ConnectionStrings:CostingConnectionString %>"
        ...
        <SelectParameters>
            <asp:ControlParameter ControlID="FormPanel$ASPxSplitter1$formLayout$CmbCompany" Name="Company" PropertyName="Value" />
            <asp:ControlParameter ControlID="FormPanel$ASPxSplitter1$formLayout$CmbPackaging" Name="Packaging" PropertyName="Text" />
            <asp:ControlParameter ControlID="FormPanel$ASPxSplitter1$formLayout$tbNetweight" Name="NetWeight" PropertyName="Text" />
            <asp:ControlParameter ControlID="FormPanel$ASPxSplitter1$formLayout$tbPackSize" Name="PackSize" PropertyName="Text" />
...
<%--    <asp:SqlDataSource ID="dsRawMaterial" runat="server" ConnectionString="<%$ ConnectionStrings:CostingConnectionString %>"
 ...
 <%--   <asp:SqlDataSource ID="ArtsDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:CostingConnectionString %>"
        ...
        DeleteCommand="DELETE FROM FileSystem WHERE ID = @Id">
        <DeleteParameters>
    ...
    <asp:SqlDataSource ID="dsComponentMat" runat="server" ConnectionString="<%$ ConnectionStrings:CostingConnectionString %>"
       SelectCommand="select * from dbo.FNC_SPLIT('Raw Material,Ingredient,Primary Packaging,',',')" />
<%--    <asp:SqlDataSource ID="dsCountry" runat="server" ConnectionString="<%$ ConnectionStrings:LabConnectionString %>"
       SelectCommand="select Id,Code, CONCAT(Description,'/',ProductGroup) as Name from [transGrade] where ProductType=case when  dbo.fnc_checktype(@usertype,'0')>0 then 'PF' else 'HF' end and IsActive=1">
        <SelectParameters>
             <asp:ControlParameter ControlID="usertp" Name="usertype" PropertyName="['usertype']"/>
        </SelectParameters>
    </asp:SqlDataSource>--%>
