...
public partial class UserControls_RequestEditForm : MasterUserControl
{
    ...
    void sendmail(DataRow dr, string keys)
    {
        ...
        string subject = string.Format(@"Request Create Material by CDTRF: {0}, {1}, {2}, {3}", CmbTechnicalNo.Text, dr["RequestNo"].ToString(), tbRefSamples.Text, tbCustomer.Text);
    ...
    protected void gridData_CustomDataCallback(object sender, ASPxGridViewCustomDataCallbackEventArgs e)
    {
        ...
        if (args[0] == "cost" || args[0] == "renew" || args[0] == "GetData")
        {
            ...
            foreach (DataRow rw in dt.Rows)
            {
                ...
                result["utype"] = string.Format("{0}", rw["UserType"]);
        ...
        if (args[0] == "GetFilteringData")
        {
            string labConn = ConfigurationManager.ConnectionStrings["LabConnectionString"].ConnectionString;
            SqlParameter[] param = new SqlParameter[] { };
            string group = cmbProductGroup.Text.Split(new char[] { ';', ':' }, StringSplitOptions.RemoveEmptyEntries)[0].Trim();
            string code = args[2];
            var dt = new DataTable();
            result["View"] = args[1];
            switch (string.Format("{0}", args[1]))
            {
                case "Grade":
                    ...
                    {
                        using (SqlCommand cmd = new SqlCommand("select Code,Description from dbo.transGrade where dbo.fnc_checktype( productgroup, @group)>0 and ProductType='PF' and code = @code union select '-1',''", conn))
                        {
                            conn.Open();
                            cmd.Connection = conn;
                    ...
                    foreach (DataRow rw in dt.Rows)
                    {
                        result["Code"] = string.Format("{0}", rw["Code"]);
                        result["Description"] = string.Format("{0}", rw["Description"]);
                    }
                    e.Result = result;
                    break;
                ...
                case "SizeMedia":
                    ...
                    {
                        result["Code"] = string.Format("{0}", rw["Code"]);
                        result["CanSize"] = string.Format("{0}", rw["CanSize"]);
                        result["Type"] = string.Format("{0}", rw["Type"]);
                        result["Media"] = string.Format("{0}", rw["Media"]);
                        result["NW"] = string.Format("{0}", rw["NW"]);
                        result["NutritionType"] = string.Format("{0}", rw["NutritionType"]);
                    }
                    e.Result = result;
                    break;
                ...
                case "ContainerLid":
                    ...
                    using (SqlConnection conn = new SqlConnection(labConn))
                    {
                        using (SqlCommand cmd = new SqlCommand("select Code, ContainerType, LidType from  dbo.transContainerLid  where dbo.fnc_checktype( productgroup, @group)>0 and code = @code and ProductType='PF'", conn))
                        {
                            conn.Open();
                            cmd.Connection = conn;
                            cmd.Parameters.AddRange(param);
                    ...
                    {
                        result["Code"] = string.Format("{0}", rw["Code"]);
                        result["ContainerType"] = string.Format("{0}", rw["ContainerType"]);
                        result["LidType"] = string.Format("{0}", rw["LidType"]);
                    }
                    e.Result = result;
                    break;
                ...
                case "Topping":
                    ...
                    using (SqlConnection conn = new SqlConnection(labConn))
                    {
                        using (SqlCommand cmd = new SqlCommand("select Code, Description from  dbo.transMediaType where dbo.fnc_checktype( productgroup, @group)>0 and code = @code and ProductType='PF'", conn))
                        {
                            conn.Open();
                            cmd.Connection = conn;
                            cmd.Parameters.AddRange(param);
                ...
                case "TransRawMaterial":
                    ...
                    using (SqlConnection conn = new SqlConnection(labConn))
                    {
                        using (SqlCommand cmd = new SqlCommand("select Code, Description from dbo.transRawMaterial where dbo.fnc_checktype( productgroup, @group)>0 and code = @code and ProductType='PF'", conn))
                        {
                            conn.Open();
                            cmd.Connection = conn;
                            cmd.Parameters.AddRange(param);
    ...
    protected void gridData_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
    {
        ...
        //    using (SqlConnection con = new SqlConnection(strConn))
        //    {
        //        string query = "update TransRequestForm set Assignee=@Assignee WHERE ID=@ID";
        //        using (SqlCommand cmd = new SqlCommand(query))
        //        {
        //            cmd.Connection = con;
        //            cmd.Parameters.AddWithValue("@Assignee", string.Join(",", arr));
