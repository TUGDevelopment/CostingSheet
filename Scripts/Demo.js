...
MailPageModule = CreateClass(PageModuleBase, {
        ...
        switch (command) {
            ...
            case "save":
                ...
                if (state.Key == 0)
                    ...
                    //if ((state.Key.split("|")[0] == 0 || state.Key.split("|")[0] > 1) && value != "") {
					if (((state.Key == 0 ? state.Key : state.Key.split("|")[0])== 0 || (state.Key == 0 ? state.Key : state.Key.split("|")[0]) > 1) && value != "") {
                    ...
                    ClientgridData.GetValuesOnCustomCallback("MailForm|" + command + "|" + state.Key, function (values) {
                        ...
                        if ((values["StatusApp"] != "0" && values["StatusApp"] != "-1" && values["StatusApp"] != "4" &&
                            values["StatusApp"] != "6" && values["StatusApp"] != "1") && (test == null || test == "0")) {
    ...
    ClientgridData_RowClick: function (s, e) {
        ...
        s.GetRowValues(e.visibleIndex, 'RequestType;UniqueColumn', function (value) {
            ClientNewID.Set('NewID', value[1]);
    ...
    ShowForm: function (command, key) {
        ...
        if (command == "new" || command == "Reply" || command == "EditDraft") {
            ...
            formLayout.GetItemByName("listclaim").SetVisible(obj==0);
            var arr = "Product_Type;Chunk_Type;Media_;Product_Stlye;Net_weight;Claims;NutrientProfile";
            arr = obj == 0 ? "Product_Requirement;SampleDate" : arr ;
            var g = arr.split(";"), s;
        ...
        if (command == "new") {
            ClientgridData.GetValuesOnCustomCallback("MailForm|" + command + "|" + key, function (values) {
                var setValuesFunc = function () {
                    ...
                    //gvzone.PerformCallback('reload|0');
                    cpUpdateFiles.PerformCallback(['reload', values["NewID"]].join("|"));
					var b=true;
                    fileManager.PerformCallback(['load',b].join("|"));
                    approv.Set("approv", values["editor"]);
                    //gvstyle.PerformCallback('reload|0');
        ...
        if (command == "Reply" || command == "EditDraft") {
            ...
            ClientgridData.GetValuesOnCustomCallback("MailForm|" + command + "|" + key, function (values) {
                var setValuesFunc = function () {
                    ...
                    if (!values)
                        return;
                    ordersGridView.PerformCallback('reload|' + values["ID"]);
                    cbRequestType.UnselectItems(cbRequestType.GetSelectedItems());
                    //TabList.GetTab(5).SetVisible(hfRole.Get("Role") != 0);
                    ...
                    //if (values["UserType"] >= 5)
                    //    gvformu.GetToolbar(0).GetItemByName("Semi").SetText("DL Frozen");
                    //else
                    //    gvformu.GetToolbar(0).GetItemByName("Semi").SetText("Semi"); 
                    ...
                    debugger;
                    Clientusertp.Set("ut",values["UserType"]);
                    ...
                    if (values["CustomerRequest"].length > 0) {
                    ...
                    //        indexes.push(parseInt(loopinner[n]));
                    //    }
                    //    ClientInner.SelectIndices(indexes);
                    //}
                    
                    //hfInner.Set("Inner", values["Inner"]);
                    //hfOuter.Set("Outer", values["Outer"]);
                    //tbInnerOther.SetText(values["InnerOther"]);
                    //if (values["Outer"] != "") {
                    //    var loopOuter = [];  
                    //    var arrOuter = values["Outer"].split(";"), o;
                    //    for (o = 0; o < arrOuter.length; o++) {
                    //        loopOuter.push(parseInt(arrOuter[o]));
                    //    //    ClientOuter.SelectIndices(loopOuter[o]);
                    //    }
                       
                    //    ClientOuter.SelectIndices(loopOuter);
                    //}
                    ...
                    if (para[0] == "Revised" || para[0] == "Copied") {
                        ...
                        ClientNewID.Set('NewID', values["NewID"]);
                        gv.PerformCallback('reload|' + values["ID"]);
                        //gvzone.PerformCallback('reload|' + values["ID"]);
                    ...
                    gvformu.PerformCallback("reload|" + values["UserType"] + "|" + values["ID"]);
                    //gvstyle.PerformCallback(['reload', values["ID"]].join("|"));
                    //ClientPage.PerformCallback('reload|' + values["ID"]);
                };
                Demo.PostponeAction(setValuesFunc, function () { return !!window.ClientRequestNo });
           });
            Demo.ShowLoadingPanel(ClientFormPanel.GetMainElement());
    ...
    ClearForm: function () {
            ...
            var arr = "Product_Type;Chunk_Type;Media_;Product_Stlye;Net_weight;Claims;NutrientProfile";
            arr = arr + ";Product_Requirement;SampleDate";
            var g = arr.split(";"), s;
            for (s = 0; s < g.length; s++) {
                if (g[s] != "")
            ...
            //gvformu.PerformCallback('symbol|1');
    },
    HideForm: function () {
...
RequestDesModule = CreateClass(PageModuleBase, {
    constructor: function () {
        this.DemoState = { View: "MailList" };
    },
    OnStateChanged: function () {
    ...
    ClientActionMenu_ItemClick: function (s, e) {
        ...
        switch (command) {
            ...
            case "save":
                ...
                Demo.ChangeDemoState("MailList");
                Demo.DoCallback(ClientgridData, function () {
                    ClientgridData.PerformCallback("Post|" + state.Key);
                });
    ...
    ShowMenuItems: function () {
        ...
        //ClientActionMenu.GetItemByName("markAs").SetVisible(view != "MailForm" && edit == "0");
        ClientActionMenu.GetItemByName("delete").SetVisible(view == "MailForm" && appr == "0" && state.Key != 1);
    ...
    ClientLayout_PaneResized: function () {
        var state = Demo.DemoState;
        if (!state) return;
        if (DevAVPageName == "RequestDesForm") {
            ClientgridData.SetHeight(0);
            ClientgridData.SetHeight(ASPxClientUtils.GetDocumentClientHeight() - TopPanel.GetHeight());
            ClientFormPanel.SetHeight(ASPxClientUtils.GetDocumentClientHeight() - TopPanel.GetHeight());
        }
    ...
    ShowForm: function (command, key) {
        Demo.HideGrid();
        Demo.ClearForm();
        //hftype.Set("type", command);
        //ClientintGrid.SetVisible(command == "unread" || command == "Edit");
        //treeList.SetVisible(command == "read");
        ClientFormPanel.SetVisible(command == "new" || command == "read" || command == "unread" || command == "Edit");
        Demo.SetUnitPriceColumnVisibility();
        if (command == "new" || command == "read" || command == "unread") {
            ClientgridData.GetValuesOnCustomCallback("MailForm|" + command + "|" + key, function (values) {
                var setValuesFunc = function () {
                    Demo.HideLoadingPanel();
                    if (!values)
                        return;
                    //treeList.PerformCallback('reload|0');
        ...
        if (command == "Edit") {
            ClientgridData.GetValuesOnCustomCallback("MailForm|" + command + "|" + key, function (values) {
                var setValuesFunc = function () {
                    ...
                    //
                    //rbRequestType.SetValue(values["RequestType"]);
                    //hfRequestType.Set("Type", values["RequestType"]);
                    hfGeID.Set("GeID", values["ID"]);
                    hfStatusApp.Set('StatusApp', values["StatusApp"]);
                    Demo.SetEnabledForm(b);
                    ...
                    ClientCustomPriceEdit.SetValue(values["Destination"]);
                    ...
                    //treeList.PerformCallback("reload|" + values["ID"]);
...
CostPageModule = CreateClass(PageModuleBase, {
    ...
    ClientInfoMenu_ItemClick: function (s, e) {
        ...
        if (command == "ExportToXLS" || command == "ExportToPDF") {
            if (state.View == "MailList" && command == "ExportToXLS")
                ...
                debugger;
                var url = "./PrintMails.aspx?view=1&Type=0&Id=" + state.Key.split("|")[0];
                window.open(url, "_blank");
    ...
    ClientActionMenu_ItemClick: function (s, e) {
        ...
        switch (command) {
            ...
            case "Approve": case "Reject":
                Demo.DoCallback(ClientCostGrid, function () {
                    ClientCostGrid.PerformCallback(["Action", command].join("|"));
                });
                Demo.MarkMessagesAsRead(true, keys);
                break
            case "Request":
                ...
                //Demo.DoCallback(ClientCostGrid, function () {
                //    ClientCostGrid.PerformCallback(["Request", command].join("|"));
                //});
                //Demo.MarkMessagesAsRead(true, keys);
                //alert('success');
                break
    ...
    ClientCostGrid_RowDblClick: function (s, e) {
        var src = ASPxClientUtils.GetEventSource(e.htmlEvent);
        if (src.tagName == "TD" && src.className.indexOf("dxgvCommandColumn") != -1) // selection cell
            return;
        if (!s.IsDataRow(e.visibleIndex))
    ...
    ShowMenuItems: function () {
        ...
        ClientActionMenu.GetItemByName("Request").SetVisible(false);
        ClientActionMenu.GetItemByName("Approve").SetVisible(false);
        ClientActionMenu.GetItemByName("Reject").SetVisible(false);
        ...
        //ClientInfoMenu.GetItemByName("Quotation").SetVisible(false);
        //ClientInfoMenu.GetItemByName("calculation").SetVisible(false);
        ...
        if (command == "New") {
            ClientCostGrid.GetValuesOnCustomCallback("MailForm|" + command + "|" +key, function (values) {
                var setValuesFunc = function () {
                    ...
                    //var b = true;
                    //fileManager.PerformCallback(['load', b].join("|"));
                    //cpUpdateFiles.PerformCallback(['load', 0].join("|"));
                    cpUpdateFiles.PerformCallback(['reload', values["NewID"]].join("|"));
        ...
        if (command == "Reply" || command == "EditDraft" || command == "EditCost") {
            ClientCostGrid.GetValuesOnCustomCallback("MailForm|" + command + "|" +key, function (values) {
                var setValuesFunc = function () {
                    ...
                    tbVarietyPack.SetText(values["VarietyPack"]);
                    usertp.Set("usertype", values["UserType"]);
...
formulaPageModule = CreateClass(PageModuleBase, {
    ...
    ShowMenuItems: function () {
        ...
        ClientActionMenu.GetItemByName("Pending").SetVisible(view == "MailList" && hftype.Get("type") == 1);
        ClientActionMenu.GetItemByName("All").SetVisible(view == "MailList" && hftype.Get("type") == 0);
        ClientActionMenu.GetItemByName("new").SetVisible(view == "MailList");
        ClientActionMenu.GetItemByName("back").SetVisible(view != "MailList");
        ClientActionMenu.GetItemByName("save").SetVisible(view == "MailForm");
        ...
        if(command == "New") {
            gridData.GetValuesOnCustomCallback("MailForm|" + command + "|" + key, function (values) {
                var setValuesFunc = function () {
                    ...
                    if (!values)
                        return;
                    hfStatusApp.Set('StatusApp', 0);
                    hfid.Set("ID", 0);
                    hfGeID.Set("GeID", 0);
                    ClientNewID.Set("NewID", values["NewID"]);
                    ClientRequestNo.SetText("###########");
                    approv.Set("approv", values["approv"]);
                    hfid.Set('hidden_value', 0);
            ...
            Demo.ShowLoadingPanel(formulaFormPanel.GetMainElement());
        }
        if (command == "Reply" || command == "EditDraft") {
            gridData.GetValuesOnCustomCallback("MailForm|" + command + "|" + key, function (values) {
                var setValuesFunc = function () {
...
CalculaModule = CreateClass(PageModuleBase, {
    ...
    ShowForm: function (command, key) {
        ...
        Demo.HideForm();
        Demo.ClearForm();
        //var toolbar = gv.GetToolbarByName("MyToolbar"); 
        //var newItem = toolbar.GetItemByName("New");  
        var NewItem = gv.GetToolbar(0).GetItemByName('New');
        NewItem.SetVisible(command == "EditDraft"?false: true);
        Upload.SetEnabled(command == "New" || command == "Clone");
        var toolbar = gv.GetToolbarByName("MyToolbar");
        if (toolbar != null) {
            toolbar.SetVisible(command == "New" || command =="Clone");
        } 
        //newItem.SetEnabled(command == "New");
        //ClientPreviewPanel.SetVisible(command == "unread" || command == "Edit");
        ClientFormPanel.SetVisible(command == "New" || command == "Reply" || command == "EditDraft" || command =="Clone");
        ...
        if (command == "New") {
            gridData.GetValuesOnCustomCallback("MailForm|" + command + "|" + key, function (values) {
                var setValuesFunc = function () {
                    ...
                    ClientValidto.SetEnabled(true);
                    ClientValidfrom.SetEnabled(true);
                    deValidityDate.SetText(addDays(7));
                    tbFreight.GetMainElement().style.borderColor = "";
                    seExchangeRate.GetMainElement().style.borderColor = "";
        ...
        if (command == "Reply" || command == "EditDraft" || command =="Clone") {
            gridData.GetValuesOnCustomCallback("MailForm|" + command + "|" + key, function (values) {
                var setValuesFunc = function () {
                    Demo.HideLoadingPanel();
                    if (!values)
                    ...
                    if (command == "Clone") {
                        ...
                        ClientValidto.SetText(myDate);
                        ClientValidto.SetEnabled(true);
                        ClientValidfrom.SetText(addDays(0));
                        ClientValidfrom.SetEnabled(true);
                        deValidityDate.SetText(addDays(7));
                        seExchangeRate.GetMainElement().style.borderColor = "";
                        CmbIncoterm.SetValue(values["Incoterm"]);
                        tbFreight.SetText(values["FreightOld"] == "" ? "0" : values["FreightOld"]);
                        //cpUpdateFiles.PerformCallback("reload|0");
                        var _b = (values["Incoterm"] == "CFR" || values["Incoterm"] == "CIF")
                        Demo.SetEnable(_b);
                        gv.PerformCallback(["Clone", key].join("|"));
                        tbFreight.GetMainElement().style.borderColor = "";
                        //Demo.SetEnable(true);
                        ...
                        ClientValidto.SetEnabled(false);
                        ClientValidfrom.SetEnabled(false);
                        
                        //cpUpdateFiles.PerformCallback(["reload", key].join("|"));
                        ClientValidfrom.SetText(values["From"]);
                        ClientValidto.SetText(values["To"]);
                        deValidityDate.SetText(values["ValidityDate"]);
                        //set color
                        tbFreight.GetMainElement().style.borderColor = values["_Freight"] > 0 ? "red" : "";
                        deValidityDate.GetMainElement().style.borderColor = values["_ValidityDate"] > 0 ? "red" : "";
                        seExchangeRate.GetMainElement().style.borderColor = values["_ExchangeRate"] > 0 ? "red" : "";
                        tbFreight.SetText(values["Freight"]);
                        OldValueFreight.Set("Freight", values["Freight"]);
                        gv.PerformCallback(["reload", key].join("|"));
                    }
                    formLayout.GetItemByName("Send_Document").SetVisible(values["editor"]==1?false:true);
                    hfid.Set("ID", values["ID"]);
                    ...
                    clientzone.SetValue(values["Zone"]);
                    ClientNotes.SetText(values["Remark"]);
                    seExchangeRate.SetText(values["ExchangeRate"]);
                    OldValueExchangeRate.Set("ExchangeRate",values["ExchangeRate"]);
                    ClientCurrency.SetText(values["Currency"]);
                    //ASPxFormLayout1.GetItemByName("Freight").SetCaption("Freight***");
                    var text = ["reload", key];
                    var arge = text.join("|");
    ...
    ClientInfoMenu_ItemClick: function (s, e) {
        ...
        if (command == "theme" || command == "help") return;
        var state = Demo.DemoState;
        if (command == "ExportToXLS" || command == "ExportToPDF" || command == "print" || command == "Quotation" || command =="calculation") {
            gridData.GetValuesOnCustomCallback("MailForm|" + command + "|" + state.Key, Demo.OnGetRowValues);
        } else
            if (e.item.parent) {
                ASPxClientUtils.SetCookie("DemoCurrentTheme", e.item.name || "");
    ...
    OnGetRowValues: function (values) {
        ...
        if (values != null) {
            ...
            var a = values["print"].split("|"), i;
            for (i = 0; i < a.length; i++) {
                url = "./PrintMails.aspx?usertype=0&view=" + view + "&Type=" + a[i] + "&Id=" + values["KeyValue"] + "&command=" + values["command"];
                window.open(url, "_blank");
            }
        }
    },
        ...
        switch (command) {
            ...
            case "save":
                ...
                if (CmbIncoterm.GetValue() == 'CIF' || CmbIncoterm.GetValue() == 'CFR') {
                    ...
                    var insertedIndicies = gv.batchEditApi.GetRowVisibleIndices();
                    for (var i = 0; i < insertedIndicies.length; i++) {
                        var SubContainers = gv.batchEditApi.GetCellValue(insertedIndicies[i], "SubContainers");
                        if (SubContainers == '')
                            return alert("Invalid Case per fcl.");
                    }
                }
                debugger;
                var assign = formLayout.GetItemByName("Assignee").GetVisible();
                if (assign && CmbAssignee.GetValue()==null)
                    return alert("Please select an approval.");
                //var edit = editor.Get("Name");
                ...
                gridData.GetValuesOnCustomCallback("MailForm|" + command + "|" + state.Key, function (values) {
                    ...
                    //    return;
                    //}
                    if (values["Mark"] != "") return alert('Please check remaining tonggol EU fish with the center');
                    Demo.ChangeDemoState("MailList");
                    Demo.DoCallback(gridData, function () {
                        gridData.PerformCallback("post|" + state.Key);
                    });
    ...
    ShowMenuItems: function () {
        ...
        var disp = appr==0 ? 'table-cell' : 'none';
        $('td.unitPriceColumn').css('display', disp);
        //
        //ClientActionMenu.GetItemByName("Approve").SetVisible(false);
        //ClientActionMenu.GetItemByName("Reject").SetVisible(view == "MailForm" && edit == "0" && appr == "0");
        ...
        ClientInfoMenu.GetItemByName("print").SetVisible(false);
        ClientInfoMenu.GetItemByName("ExportToXLS").SetVisible(false);
    }
});
QuotaPageModule = CreateClass(PageModuleBase, {
    ...
    ClientActionMenu_ItemClick: function (s, e) {
        ...
        switch (command) {
            ...
            case "save":
                ...
                gridData.GetValuesOnCustomCallback("MailForm|" + command + "|" + state.Key, function (values) {
                    if ((_Disp == 3 || _Disp == 5 || _Disp == 7) && (ClientReason.GetText() == "")) {
                        switch (_Disp) {
                            case "3":
                                alert('The Reason should not be empty...');
                                break;
            ...
            case "Approve": case "Reject":
                Demo.DoCallback(gridData, function () {
                    gridData.PerformCallback(["Action",command].join("|"));
                });
                Demo.MarkMessagesAsRead(true, keys);
                break
            case "delete":
    ...
    ClearForm: function () {
        ...
        tbFreight.SetText("");
        CmbIncoterm.SetSelectedIndex(-1);
        ClientBrand.SetSelectedIndex(-1);
        CmbRoute.SetSelectedIndex(-1);
        CmbSize.SetSelectedIndex(-1);
        tbInsurance.SetText("");
        ClientCustomer.SetSelectedIndex(-1);
        ...
        if (command == "Reply" || command == "EditDraft") {
            gridData.GetValuesOnCustomCallback("MailForm|" + command + "|" + key, function (values) {
                var setValuesFunc = function () {
                    ...
                    ClientBrand.SetValue(values["Brand"]);
                    CmbPaymentTerm.SetValue(values["PaymentTerm"]);
                    seExchangeRate.SetValue(values["ExchangeRate"]);
...
VarietyModule = CreateClass(PageModuleBase, {
    constructor: function () {
        this.DemoState = { View: "MailForm" };
    },
    OnStateChanged: function () {
        var state = this.DemoState;
        if (state.View == "MailForm")
...
RequestPageModule = CreateClass(PageModuleBase, {
    constructor: function () {
        this.DemoState = { View: "MailList" };
    },
    gridData_RowClick: function (s, e) {
    ...
    OnEndCallback: function (s, e) {
        if (!ClientgridData.cpKeyValue)
            return;
        if (ClientgridData.cpKeyValue == 0)
            alert(ClientgridData.cpKeyValue);
        var key = ClientgridData.cpKeyValue;
        ClientgridData.cpKeyValue = null;
        if (key == 0)
            Demo.ChangeDemoState("MailList");
        else {
            var args = key.split("|");
    ...
    ClientLayout_PaneResized: function () {
        if (DevAVPageName == "RequestEditForm") {
            ClientgridData.SetHeight(0);
            ClientgridData.SetHeight(ASPxClientUtils.GetDocumentClientHeight() - TopPanel.GetHeight());
            ClientFormPanel.SetHeight(ASPxClientUtils.GetDocumentClientHeight() - TopPanel.GetHeight());
        }
    ...
    ClientInfoMenu_ItemClick: function (s, e) {
        ...
        if (command == "ExportToXLS" || command == "ExportToPDF") {
            if (state.View == "MailList" && command == "ExportToXLS")
                ...
                if (state.Key == 0) return alert("data not found");
                var url = "./PrintMails.aspx?view=5&Type=3&Id=" + state.Key;
                //alert(url);
                window.open(url, "_blank");
                ClientInfoMenu.GetItemByName('print').SetNavigateUrl(url);
            }
    ...
    GetSelectedFieldValuesCallback: function (values) {
        ...
        for (var i = 0; i < values.length; i++) {
            arr1.push(values[i]);
            var url = "./PrintMails.aspx?view=5&Type=3&Id=" + values[i];
            window.open(url, "_blank");
            ClientInfoMenu.GetItemByName('print').SetNavigateUrl(url);
        }
    },
    ClientActionMenu_ItemClick: function (s, e) {
        ...
        switch (command) {
            ...
            case "Approve":
                Demo.ChangeDemoState("MailList");
                Demo.DoCallback(ClientgridData, function () {
                    ClientgridData.PerformCallback("Approve|" + state.Key);
                });
                ClientgridData.UnselectRows();
                break;
            case "new":
                Demo.ChangeDemoState("MailForm", "New", 0);
                break;
    ...
    ClearForm: function () {
        ...
        ClientReceiver.SetValue("");
        ClientRequestNo.SetText("###########");
        ClientHierarchy.SetText("");
        ClientResult.SetText(null);
        ClientProductCode.SetText("");
        ClientProductGroup.SetSelectedIndex(-1);
        ClientRequestDate.SetText(Demo.currentdate());
        ClientNotes.SetText("");
        ClientRawMaterial.SetSelectedIndex(-1);
        ClientStyle.SetSelectedIndex(-1);
        ClientTopping.SetSelectedIndex(-1);
        ClientSizeMedia.SetSelectedIndex(-1);
        ClientContainerLid.SetSelectedIndex(-1);
        ClientPackingStyle.SetText("")
        ClientGrade.SetSelectedIndex(-1);
        ClientZone.SetSelectedIndex(-1);
        ClientDestination.SetSelectedIndex(-1);
        ClientRefSamples.SetText("")
        ClientCountry.SetText("")
        ClientPetType.SetSelectedIndex(-1);
        ClientDivision.SetSelectedIndex(-1);
        ClientNutrition.SetSelectedIndex(-1);
        hfCostID.Set("CostID", 0);
        hfTrfID.Set("TrfID", 0);
        ClientTechnicalNo.SetText("");
        ClientMarketingNo.SetValue("");
        ClientgridData.UnselectRows();
        formLayout.GetItemByName("Reason").SetVisible(false);
        formLayout.GetItemByName("Assignee").SetVisible(false);
            ...
            ClientgridData.GetValuesOnCustomCallback("MailForm|" + command + "|" + key, function (values) {
                var setValuesFunc = function () {
                    ...
                    if (!values)
                        return;
                    ClientDisponsition.PerformCallback('0');
                    hfGeID.Set("GeID", values["NewID"]);
                    editor.Set("Name", 0);
                    ClientPackingStyle.SetText('00');
                };
                Demo.PostponeAction(setValuesFunc, function () { return !!window.ClientRequestNo });
            });
            Demo.ShowLoadingPanel(ClientFormPanel.GetMainElement());
            ...
            ClientgridData.GetValuesOnCustomCallback("MailForm|" + command + "|" + key, function (values) {
                var setValuesFunc = function () {
                    ...
                    if (!values)
                        return;
                    clienteventlog.SetValue(values["Eventlog"]);
                    ClientReceiver.SetValue(values["Assignee"]);
                    ClientCustomer.SetText(values["Customer"])
                    hfGeID.Set("GeID", values["NewID"]);
                    hfCostID.Set("CostID", values["CostID"]);
                    hfTrfID.Set("TrfID", values["TRFID"]);
                    ClientRequestNo.SetText(values["DocumentNo"]);
                    ClientTechnicalNo.SetValue(values["RequestNo"]);
                    ClientMarketingNo.SetValue(values["Costno"]);
                    ClientProductGroup.SetValue(values["ProductGroup"]);
                    ClientRequestDate.SetText(values["RequestDate"]);
                    ClientNotes.SetText(values["Remark"]);
                    ClientResult.SetText(values["Code"]);

                    ClientDestination.SetValue(values["Destination"]);
                    ClientRefSamples.SetText(values["RefSamples"]);
                    ClientCountry.SetText(values["Country"]);
                    ClientPetType.SetValue(values["PetType"]);
                    ClientDivision.SetValue(values["Division"]);
                    ClientNutrition.SetValue(values["Nutrition"]);

                    ClientPackingStyle.SetText(values["PackingStyle"]);

                    var str = values["RequestNo"];
                    Demo.setproduct(values);
                    //Demo.DoCallback(ClientFormPanel, function () {
                    //    ClientFormPanel.PerformCallback("build|"+key);
                    //});
    ...
    setproduct: function (values) {
        //debugger;
        ClientRawMaterial.PerformCallback([values["Group"], values["RawMaterial"]].join('|'));
        ClientStyle.PerformCallback([values["Group"], values["StyleofPack"]].join('|'));
        ClientTopping.PerformCallback([values["Group"], values["MediaType"]].join('|'));
        ClientSizeMedia.PerformCallback([values["Group"], values["NW"]].join('|'));
        ClientContainerLid.PerformCallback([values["Group"], values["ContainerLid"]].join('|'));
        ClientGrade.PerformCallback([values["Group"], values["Grade"]].join('|'));
        ClientZone.PerformCallback([values["Group"], values["Zone"]].join('|'));
    },
    MarkMessagesAsRead: function (read, keys) {
        var sendCallback = false;
    },
...
QuotationModule = CreateClass(PageModuleBase, {
    constructor: function () {
        this.DemoState = { View: "MailList" };
    },
    OnStateChanged: function () {
    ...
    ClientLayout_PaneResized: function () {
        if (DevAVPageName == "ReportQuotation") {
            gridData.SetHeight(0);
            gridData.SetHeight(ASPxClientUtils.GetDocumentClientHeight() - TopPanel.GetHeight());
        }
    },
...
EntryPageModule = CreateClass(PageModuleBase, {
    ...
    ShowMenuItems: function () {
        ...
        ClientActionMenu.GetItemByName("back").SetVisible(view != "MailList");
        ClientActionMenu.GetItemByName("new").SetVisible(view != "MailForm");
        //ClientActionMenu.GetItemByName("markAs").SetVisible(view != "MailForm");
        ClientActionMenu.GetItemByName("delete").SetVisible(view != "MailForm");
        ClientInfoMenu.GetItemByName("print").SetVisible(false);
    },
    ClientActionMenu_ItemClick: function (s, e) {
...
(function () {
    ...
    else if (DevAVPageName == "ReportQuotation")
        pageModule = new QuotationModule();
    else if (DevAVPageName == "RequestDesForm")
        pageModule = new RequestDesModule();
    else if (DevAVPageName == "RequestEditForm")
        pageModule = new RequestPageModule();
    else
