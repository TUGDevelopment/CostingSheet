CREATE PROCEDURE [dbo].[spGetFillWeight] 
@Code nvarchar(max)
AS
BEGIN
...
set @name = isnull((select top 1 name from MasFishSpecies where sapcode = substring(@Code,3,2)),'None')
...
SELECT *,convert(nvarchar(max),'') Yield into #fw FROM  @fw
 --select * from stdTunaFixFW
...
declare @Styles nvarchar(max),
	...
	update #fw set Yield=@Yeild,SHD=@Styles,FishGroup=@FishGroup,FishCert=isnull(@FishCert,'0') where RowID=@RowID
	FETCH NEXT FROM cur_fw INTO @FishGroup, @SHD, @FishCert,@RowID
END
