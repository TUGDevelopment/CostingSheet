...
-- =============================================
create PROCEDURE [dbo].[spGetUpdateCost2]
	@GeID nvarchar(max),
	@user nvarchar(max) ,
	@param nvarchar(max) 
AS
BEGIN
	...
	--select FORMAT(@x, 'yy')
	set @runid = (select fn from MasCompany where substring(code,1,3)=@Company)+FORMAT(@x, 'yy')+(
	...
	@TechnicalID,@runid,RDNumber,Company,PackSize,getdate(),@user,null,null,@myid,Remark,CanSize,Packaging,2,0
      ,isnull((select top 1 Rate from MasExchangeRat where Company=@Company and getdate() between Validfrom and Validto),0) as 'ExchangeRate'
	  ,Netweight,0,0,customer --ExchangeRate
