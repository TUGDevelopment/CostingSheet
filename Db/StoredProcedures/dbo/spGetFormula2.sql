...
-- =============================================
create Procedure [dbo].[spGetFormula2]
...
BEGIN
	...
	--declare @Data nvarchar(max)=39903,@user nvarchar(max)='FO5910155'
	--select * from TransFormula where RequestNo=@Data
	SET NOCOUNT ON;
	If Object_ID('tempdb..#CostingHeader')  is not null  drop table #CostingHeader
    select * into #CostingHeader from TransCostingHeader where ID=@Data
	-- Insert statements for procedure here
	declare @PackSize float,@usertype nvarchar(max)
	select @usertype=UserType from #CostingHeader

	If Object_ID('tempdb..#find')  is not null  drop table #find
	select * into #find from dbo.fnc_UserType(@user,@usertype) where idx in(2,3,0)

	if(select ISNUMERIC(PackSize) from #CostingHeader where ID=@Data)=0
	set @PackSize = 1;
	else
	set @PackSize =(select PackSize from #CostingHeader where ID=@Data)
