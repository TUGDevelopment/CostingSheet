...
-- =============================================
CREATE Procedure [dbo].[spinsertFormulaHeader] 
...
BEGIN
	...
	if (select count(*) from TransFormula where Formula=@formula and RequestNo=@RequestNo)>0 begin
	...
	begin
		...
		--select FORMAT(1,'00') 
		--print @CostNo;
	if (select count(*) from TransFormulaHeader where Formula=@formula and RequestNo=@RequestNo)>0 
		...
		values(@name,@Customer,@Code,@RefSamples,@formula,0,@Costper,@CostNo,@Revised,@MinPrice,@PID,@SellingUnit,@ref,@Packaging,@nw,@PackSize,@RequestNo)
	end
	else 
	update TransFormulaHeader set Name=@name,RefSamples=@RefSamples,IsActive=@IsActive,Code=@Code,MinPrice=@MinPrice,SellingUnit=@SellingUnit,[ref]=@ref,Packaging=@Packaging,
	nw=@nw,PackSize=@PackSize
...
--select * from TransFormulaHeader where RequestNo=18461
