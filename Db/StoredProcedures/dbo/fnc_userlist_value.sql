...
-- =============================================
Create FUNCTION [dbo].[fnc_userlist_value](
...
BEGIN
	...
	select empid from(select empid,[FirstName]
	  ...
	  where a.EmpId in (select [user_name] from ulogin where dbo.fnc_checktype(usertype,@usertype)>0)
	  and dbo.fnc_checktype(@Bu,Bu)>0)#a where #a.Sublevel in (
