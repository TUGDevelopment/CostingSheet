...
-- =============================================
CREATE PROCEDURE [dbo].[spGetRequestRate]
...
--declare @id nvarchar(max) = '103|5536A1HGNN01|7991',@from datetime='20210101',@to datetime='20210601'
...
PIVOT(MAX(Item) FOR ItemNumber IN ([1],[2],[3])) as t3) #a
	...
	SET NOCOUNT ON;
	declare @int int,@dt nvarchar(50),@valid datetime,@diff int,@exchangeRat float,@customer nvarchar(50),@usertype nvarchar(max)
	...
	)and IsActive=0  and (select count(value) from dbo.FNC_SPLIT(BU,';') where value =@usertype)>0 
	...
	begin 
		...
		insert into @MyTableVar
 		select Company,material,[Description],replace(case when @dt < 7 then PriceStd1
