...
-- =============================================
CREATE Procedure [dbo].[spSplitTechnical] 
...
BEGIN
	...
	--declare @RequestNo nvarchar(50)='8061',@Requester nvarchar(250)='fo5910155',@RequestType nvarchar(250)='0',@usertype nvarchar(max)='2'
	SET NOCOUNT ON;
	...
	--set @myid =(SELECT CONVERT(nvarchar(max), NEWID()));
	...
	set @Count =(select count(*)from TransTechnical where RequestNo=@X and StatusApp <>4)
	--print @Count;
	...
	declare @fn nvarchar(max),@oldvalue nvarchar(max)
	declare cur_table CURSOR FOR
	SELECT value,@RequestNo FROM dbo.FNC_SPLIT('TransProductList,TransApprove',',')
	open cur_table
	FETCH NEXT FROM cur_table INTO @fn,@oldvalue
	...
	If Object_ID('tempdb..#temp')  is not null  drop table #temp
		...
		SET @cols = STUFF((SELECT ',' + QUOTENAME(c.column_name) 
			...
			,1,1,'')
			set @query='insert '+@fn+' select '+@cols+','+@ID+' from '+@fn+' where RequestNo='+@oldvalue
			print @query;
			execute(@query)
			FETCH NEXT FROM cur_table INTO @fn,@oldvalue
	END
...
--
 ...
 --select * FROM TransTechnical where RequestNo in ('10102200845') 
 --select * from TransProductList	where RequestNo=8069
go
