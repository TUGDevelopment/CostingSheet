...
-- =============================================
create Procedure [dbo].[spselectrequest3]
@Id nvarchar(max),
@username nvarchar(max)
--@compa nvarchar(10)
AS
BEGIN
	--declare @Id nvarchar(max)='2867',@username nvarchar(max)='FO5910155'
	--intial
	...
	---print @statusapp;
	 ...
	 where RequestNo=@requestNo and StatusApp in (0,1,2,5,9,10)) --approve Copied Status
	---authorization
	...
	when @statusapp in (6) and (select count(*) from #temp where 2 = idx) >0 then
		0
...
--	,case StatusApp when 6 then 1 else @editor end as 'editor'
    ...
    ,isnull(PrdRequirement,'')'PrdRequirement'
	,case when SecInner='' then '|' when SecInner is null then isnull(SecInner,'|') else SecInner end 'Inner'
	,isnull(SecOuter,'')'Outer'
	 from TransTechnical  where ID=@Id
END
